...
function renderOnePara(
  ...
  p.className = 'para' + (whole ? '' : ` frag${s > 0 ? ' cont-top' : ''}${e < total ? ' cont-bottom' : ''}`);
  ...
  for (let ti = s; ti < e; ti++) {
    ...
    w.addEventListener('click', (ev) => {
      ...
      root?.querySelectorAll('.tok.selected, .token.tapped').forEach((n) => n.classList.remove('selected', 'tapped'));
