...
const lazy = /lang-dict|epub|llm|url-ingest|markdown|marked|mobi/;
const first = rows.filter((r) => !lazy.test(r.f));
const total = first.reduce((a, r) => a + r.gzip, 0);
console.log("--- chunks (gzip) ---");
for (const r of rows)
  console.log(
    `${lazy.test(r.f) ? "[lazy] " : "[first]"}${r.f} raw=${(r.raw / 1024).toFixed(1)}KB gzip=${(r.gzip / 1024).toFixed(1)}KB`
  );
console.log(
  `首屏合计 gzip=${(total / 1024).toFixed(1)}KB（上限 ${LIMIT_KB}KB；懒加载块不计但不得被 modulepreload）`
);
