...
(globalThis as Record<string, unknown>).fetch = async (url: string) => {
  ...
  try {
    const body = readFileSync(
      resolve(process.cwd(), "..", "..", "public", "dict", m[1], `${m[2]}.dict`),
      "utf8"
    );
...
check(
  ...
  JSON.stringify(book.chapters.map((c) => c.title))
);
...
check(
  "md 去链接URL",
  !mdText.includes("example.com") && mdText.includes("Time"),
  mdText.slice(0, 80)
);
...
const art = extractArticle(
  "<html><body><article><p>" + "word ".repeat(30) + "</p><p>short</p></article></body></html>"
);
...
const urlErr = await fetchArticle("https://example.com/x", "en").then(
  () => "",
  (e: Error) => e.message
);
...
check(
  "hideKnown 隐藏已知词",
  shouldShowGloss(hello, { hideStopwords: false, hideKnown: true, freqHideTopN: 0 }, rank) === false
);
const the = tokenizeParagraph(pack, "en", "The cat", () => false).find((t) => t.lemma === "the")!;
the.gloss = "这";
check(
  "hideStopwords 隐藏停用词",
  shouldShowGloss(the, { hideStopwords: true, hideKnown: false, freqHideTopN: 0 }) === false
);
...
{
  const KJV_TEXT =
    "He hath made the house.\n\nHe doth read the book.\n\nHe saith the word is good.";
  const kjv = parseTxt(KJV_TEXT, "kjv-2samuel.txt", "en");
  check(
    "kjv 章节切分=1章3段",
    kjv.chapters.length === 1 && kjv.chapters[0].paragraphs.length === 3,
    JSON.stringify(kjv.chapters[0]?.paragraphs)
  );
  ...
  for (const target of ["zh", "en"] as const) {
    ...
    check(
      `kjv 小节注出率>=0.8(${target})`,
      rate >= 0.8,
      `rate=${hits}/${toks.length}=${rate.toFixed(2)} ${toks
        .filter((t) => !t.gloss)
        .map((t) => t.lemma)
        .join(",")}`
    );
...
// 根因：src/ui/app.ts 顶部 pager 在 body 之后 append，两套全挤底部 + 跨 await 旧续体重复挂 bottom。
{
  ...
  check(
    "单页仅保留底部 pager",
    topMounts === 0 && bottomMounts === 1,
    `top=${topMounts} bottom=${bottomMounts}`
  );
  ...
  check(
    "pager 顺序 body<bottom",
    bodyIdx !== -1 && bottomIdx !== -1 && bodyIdx < bottomIdx,
    `${bodyIdx}/${bottomIdx}`
  );
  ...
  check(
    "单页 pager 数量==2",
    main.querySelectorAll(".pager").length === 2,
    `got ${main.querySelectorAll(".pager").length}`
  );
  check(
    "top/bottom 各一套",
    main.querySelectorAll('[data-testid="pager-top"]').length === 1 &&
      main.querySelectorAll('[data-testid="pager-bottom"]').length === 1
  );
}
...
// 目标：桌面对开两栏、移动单栏；页内分页，末页下页进下一章首夜、首夜上页回上一章末页。
{
  ...
  check(
    "对开容器 book-spread",
    (src.includes("book-spread") && src.includes('data-testid", "book-spread')) ||
      src.includes("data-testid', 'book-spread"),
    "app.ts 缺 book-spread 容器"
  );
  check(
    "对开左右页 book-page",
    src.includes("book-page-left") && src.includes("book-page-right"),
    "缺左右页"
  );
  check(
    "章列表 chapter-list",
    src.includes("chapter-list") && src.includes("chapter-item-"),
    "缺章列表"
  );
  check(
    "章标记 data-testid chapter",
    src.includes('data-testid", "chapter"') || src.includes("data-testid', 'chapter'"),
    "缺章标记"
  );
  check(
    "章内页码 1/N 保留",
    src.includes("/${totalPages}") && src.includes("state.page + 1"),
    "pager info 须保留 章内 page+1/totalPages"
  );
  // --- 源码口径：章间连续翻页（末页 next 进章 / 首夜 prev 回章，全书首末才禁用）---
  check(
    "跨章 next 进章",
    src.includes("state.chapterIdx += 1") && src.includes("state.page = 0"),
    "缺末页 next 进下一章"
  );
  check(
    "跨章 prev 回章",
    src.includes("state.chapterIdx -= 1") && src.includes("totalPagesFor(state.chapterIdx) - 1"),
    "缺首夜 prev 回上一章末页"
  );
  check(
    "全书首末才禁用",
    src.includes("isFirstOfBook") && src.includes("isLastOfBook"),
    "须全书首/末才禁用，不按章内禁用"
  );
  // --- CSS 口径：移动单栏 + 桌面双栏（响应式无内滚）---
  check(
    "CSS 对开容器",
    css.includes(".book-spread") && css.includes(".book-page"),
    "styles.css 缺对开样式"
  );
  check(
    "CSS 响应式无内滚视口",
    css.includes(".book-spread") &&
      css.includes("grid-template-columns: 1fr 1fr") &&
      css.includes("overflow: hidden") &&
      css.includes("break-inside: avoid"),
    "对开必须 overflow:hidden + 双栏grid + break-inside:avoid（无内滚，单段超长例外才内滚）"
  );
  check(
    "CSS 单段超长例外",
    css.includes("single-para") && css.includes("overflow: auto"),
    '须保留 data-overflow="single-para" 例外才 overflow:auto'
  );
  check(
    "CSS 无固定视口 clamp",
    !css.includes("clamp(420px") &&
      !css.includes("100vh - 190px") &&
      !css.includes("100vh - 280px"),
    "须删除固定 clamp/100vh-190px/280px，改 flex:1 自适应"
  );
  check(
    "无固定词数分页",
    !src.includes("WORDS_PER_SPREAD"),
    "须删除 WORDS_PER_SPREAD，改按实测高度分页"
  );
  ...
  // 章列表 DOM
  const chapters = [
    { title: "Morning", paragraphs: ["a", "b", "c", "d", "e", "f"] },
    { title: "Night", paragraphs: ["g", "h"] },
  ];
  ...
  check(
    "DOM 章项数量==章数",
    main.querySelectorAll('[data-testid^="chapter-item-"]').length === chapters.length
  );
  check("DOM 章标记存在", main.querySelectorAll('[data-testid="chapter"]').length === 1);
  check(
    "DOM 对开容器存在",
    main.querySelectorAll('[data-testid="book-spread"].book-spread').length === 1
  );
  check(
    "DOM 对开左右页各一",
    main.querySelectorAll('[data-testid="book-page-left"]').length === 1 &&
      main.querySelectorAll('[data-testid="book-page-right"]').length === 1
  );
  check(
    "DOM pager+chapter 共存",
    main.querySelector('[data-testid="chapter-list"]') != null &&
      main.querySelector('[data-testid="chapter"]') != null &&
      main.querySelector('[data-testid="book-spread"]') != null
  );
  // --- DOM 口径：章间连续翻页逻辑（与 app.ts 同语义的最小复刻 + 文本 1/N 断言）---
  const pageSize = 5;
  const totalPagesFor = (ci: number): number =>
    Math.max(1, Math.ceil(chapters[ci].paragraphs.length / pageSize));
  let ci = 0;
...
{
  const { buildEpubFiles, buildEpubBlob, epubFilename, renderTokenEpub } =
    await import("../src/export/epub.js");
  const { parseTxt: parseTxt2 } = await import("../src/ingest/txt.js");
  // 10a) 文件名含语言
  check(
    "epub 文件名含语言",
    epubFilename("Hello", "ja", "zh") === "Hello-ja-zh.epub",
    epubFilename("Hello", "ja", "zh")
  );
  check(
    "epub 文件名含en目标",
    epubFilename("Hi", "en", "en").endsWith("-en-en.epub"),
    epubFilename("Hi", "en", "en")
  );
  ...
  check(
    "epub 有注落小字括号",
    hitHtml.includes("<small") && hitHtml.includes("家") && /（家）/.test(hitHtml),
    hitHtml
  );
  ...
  // 回归：は→feather / ま→just… / 出る思う書く聞く→到 / 開ければ→開く。
  {
    ...
    for (const t of missJa) {
      for (const target of ["zh", "en"] as const) {
        ...
        check(
          `诚实 miss ja/${target}/${t}`,
          r.gloss === null && r.source === null,
          JSON.stringify(r.gloss)?.slice(0, 60)
        );
      }
    }
    ...
    check(
      "诚实 思う->zh 非到（pivot 不串味，缺词走LLM）",
      rDao.gloss === null || !rDao.gloss.includes("到"),
      JSON.stringify(rDao.gloss)?.slice(0, 60)
    );
    // 阳性对照：真命中不受影响（桥还在）。
    ...
    check(
      "多义项 glosses 全量",
      rAll.glosses.length >= 4 && rAll.gloss === rAll.glosses[0],
      `n=${rAll.glosses.length}`
    );
    const annA = await annotateParagraphs(pack, "en", "zh", ["a book"], () => false);
    const tokA = annA[0].tokens.find((t) => t.lemma === "a");
    check(
      "多义项 Token 携带",
      !!tokA?.glosses && tokA.glosses.length >= 4 && tokA.gloss === tokA.glosses[0],
      `n=${tokA?.glosses?.length}`
    );
  }
  // 10c) 章节保留 + 标准结构（mimetype/container/opf/ncx/ch）
  const demo = parseTxt2("第一段家。\n\n第二段町。", "demo.txt", "ja");
  const packJa = getFallbackPack("ja");
  const ann1 = await annotateParagraphs(
    packJa,
    "ja",
    "zh",
    demo.chapters[0].paragraphs,
    () => false
  );
  const files = await buildEpubFiles(demo, [ann1], "zh");
  check("epub 含mimetype", files.get("mimetype") === "application/epub+zip");
  check(
    "epub 含container",
    String(files.get("META-INF/container.xml") ?? "").includes("content.opf")
  );
  check(
    "epub 含opf+ncx",
    String(files.get("OEBPS/content.opf") ?? "").includes("ch1") &&
      String(files.get("OEBPS/toc.ncx") ?? "").includes(demo.title)
  );
  check(
    "epub 章节数==书章数",
    [...files.keys()].filter((k) => /^OEBPS\/ch\d+\.xhtml$/.test(k)).length === demo.chapters.length
  );
  ...
  check(
    "epub opf语言含目标",
    String(files.get("OEBPS/content.opf") ?? "").includes("ja-zh"),
    "opf lang"
  );
  ...
  check(
    "阅读页导出按钮",
    appSrc.includes('data-testid", "export-epub') || appSrc.includes("data-testid', 'export-epub"),
    "缺 reader export-epub"
  );
  check(
    "设置页导出按钮",
    appSrc.includes("export-epub-settings"),
    "缺 settings export-epub-settings"
  );
  check(
    "导出复用JSZip",
    appSrc.includes("import('../export/epub.js')") &&
      readFileSync(resolve(process.cwd(), "src/export/epub.ts"), "utf8").includes(
        "import('jszip')"
      ),
    "须动态 import jszip"
  );
  check(
    "无key纯词典",
    appSrc.includes("纯词典导出") || appSrc.includes("纯词典"),
    "缺无 key 纯词典文案"
  );
  check("有key回填", appSrc.includes("LLM 回填") || appSrc.includes("回填"), "缺有 key 回填逻辑");
  check(
    "B模式不调LLM",
    appSrc.includes("mode !== Mode.B") || appSrc.includes("Mode.B"),
    "导出须 B 模式不调 LLM"
  );
  ...
  try {
    ...
    check(
      "epub zip含章节",
      names.some((n) => n === "OEBPS/ch1.xhtml"),
      names.join(",")
    );
  ...
  check(
    "ja notice日→中LLM为主",
    appSrc.includes("日→中") &&
      (appSrc.includes("缺词可用 LLM") || appSrc.includes("缺词直走 LLM")),
    "app.ts 缺 ja->zh 提示"
  );
}
...
//   合格 = 综合>=0.7 且 命中>=0.6 且 中文>=0.7 且 干净>=0.9；强模型复核默认关。
{
  ...
  const { scoreQuality, suggestModel, JUDGE_DEFAULT_OFF } =
    await import("../../provider/golden/score.js");
  const { runQualityTest } = await import("../src/llm/quality.js");
  type GoldT = Array<{
    id: string;
    tokens: Array<{ i: number; lemma: string }>;
    refs: Array<{ lemma: string; expects: string[] }>;
  }>;
  const gold = GOLDEN_SENTENCES as unknown as GoldT;
  check("金标 7 语（en/de/fr/it/es/ru/ja）", gold.length === 7, `got ${gold.length}`);
  check(
    "金标 target 全 zh",
    (GOLDEN_SENTENCES as Array<{ target: string }>).every((g) => g.target === "zh")
  );
  ...
  check(
    "MOCK 建议含换模型",
    /命中率低/.test(suggestModel(mockScore)),
    suggestModel(mockScore).slice(0, 40)
  );
  ...
  const zhFetch = (async (_url: string, init?: { body?: string }) => {
    ...
    const payload = JSON.parse(user) as {
      sentences: Array<{ id: string; tokens: Array<{ i: number; lemma: string }> }>;
    };
    const refMap = new Map(
      gold.map((g) => [g.id, new Map(g.refs.map((r) => [r.lemma.toLowerCase(), r.expects[0]]))])
    );
  ...
  check(
    "中文桩判合格",
    zhReport.pass === true && zhReport.verdict === "合格",
    `${zhReport.verdict} ${zhReport.overall.toFixed(3)}`
  );
  ...
  try {
    await runQualityTest(
      { baseUrl: "https://llm.example.com/v1", apiKey: "", model: "m" },
      zhFetch
    );
  } catch {
    noKey = true;
  ...
  check(
    "复核默认关（unchecked）",
    appSrcQ.includes("quality-judge") && appSrcQ.includes("judgeBox.checked = false"),
    "复核须默认关"
  );
  check(
    "UI 不渲染金标原文/参考",
  ...
  check(
    "DOM quality-test 可断言",
    qdoc.querySelectorAll('[data-testid="quality-test"]').length === 1
  );
  check(
    "DOM quality-result 可断言",
    qdoc.querySelectorAll('[data-testid="quality-result"]').length === 1
  );
  check(
    "DOM quality-judge 默认关",
    (qdoc.querySelector('[data-testid="quality-judge"]') as HTMLInputElement).checked === false
  );
}
...
//       UI 语言下拉末尾 Auto + Mode 强制走 LLM（B 亦不例外）+ notice 明示纯 LLM 费用。
{
  ...
  check(
    "auto 为末位语言",
    SOURCE_LANGS[SOURCE_LANGS.length - 1] === "auto",
    SOURCE_LANGS.join(",")
  );
  check(
    "auto 显示名含 Auto/万能",
    /Auto/.test(SOURCE_LANG_NAMES["auto" as never]) &&
      /万能/.test(SOURCE_LANG_NAMES["auto" as never]),
    SOURCE_LANG_NAMES["auto" as never]
  );
  // ingest：auto 可导入（TXT 章节保留，语言=auto）
  const autoBook = parseTxt(`${AR}\n\nصباح الخير`, "ar.txt", "auto" as never);
  check(
    "auto TXT 可导入",
    autoBook.lang === "auto" && autoBook.chapters[0].paragraphs.length === 2,
    autoBook.lang
  );
  ...
  const autoToks = tokenizeParagraph(autoPack, "auto" as never, AR, () => false).filter(
    (t) => t.isWord
  );
  ...
  check(
    "auto 与 en 键不同槽",
    autoSentKey !== enSentKey,
    `${autoSentKey.slice(0, 8)} vs ${enSentKey.slice(0, 8)}`
  );
  ...
  check(
    "auto 键与 sha1 对拍",
    autoSentKey === expectAuto,
    `${autoSentKey.slice(0, 12)} vs ${expectAuto.slice(0, 12)}`
  );
  ...
  const c1 = { n: 0 };
  const r1 = await autoSegmentGloss(
    { baseUrl: "https://llm.example.com/v1", apiKey: "sk-test", model: "m" },
    "zh",
    AR,
    mkFetch(autoJson, c1)
  ...
  check(
    "auto 分词+注（zh）",
    r1.tokens.length === 2 && r1.tokens[0].gloss === "你好" && r1.detectedLang === "ar",
    JSON.stringify(r1.tokens)
  );
  check("auto 首调计费", r1.costUSD > 0 && c1.n === 1, `cost=${r1.costUSD} n=${c1.n}`);
  const r1b = await autoSegmentGloss(
    { baseUrl: "https://llm.example.com/v1", apiKey: "sk-test", model: "m" },
    "zh",
    AR,
    mkFetch(autoJson, c1)
  ...
  const autoEnJson = JSON.stringify({
    sentences: [{ id: "s1", tokens: [{ i: 0, surface: "مرحبا", lemma: "مرحبا", gloss: "hello" }] }],
  });
  const rEn = await autoSegmentGloss(
    { baseUrl: "https://llm.example.com/v1", apiKey: "sk-test", model: "m" },
    "en",
    "مرحبا صباحا",
    mkFetch(autoEnJson, { n: 0 })
  );
  check("auto 目标 en 可切", rEn.tokens[0].gloss === "hello");
  let noKeyAuto = false;
  try {
    await autoSegmentGloss(
      { baseUrl: "https://llm.example.com/v1", apiKey: "", model: "m" },
      "zh",
      AR,
      mkFetch(autoJson, { n: 0 })
    );
  } catch {
    noKeyAuto = true;
  }
  ...
  check(
    "provider 有 autoSegmentGloss",
    provSrc.includes("export async function autoSegmentGloss"),
    "缺 autoSegmentGloss"
  );
  check(
    "provider 缓存键 auto 槽",
    provSrc.includes("sentenceCacheKey('auto'"),
    "须 sentenceCacheKey('auto'…)"
  );
  ...
  check(
    "UI 调 autoSegmentGloss",
    appSrcAuto.includes("autoSegmentGloss"),
    "阅读/导出须调 autoSegmentGloss"
  );
  check("UI 不经词典/分词包", appSrcAuto.includes("不经词典/分词包"), "缺 auto 旁路注释");
  check(
    "UI Mode 强制走 LLM",
    appSrcAuto.includes("book.lang === 'auto'") && appSrcAuto.includes("Mode.B"),
    "auto 须 book.lang 分支 + B 亦走 LLM"
  );
  check(
    "UI 无 key 导设置",
    appSrcAuto.includes("Auto 万能模式需 LLM Key") && appSrcAuto.includes("state.tab = 'settings'"),
    "无 key 须报错 + 导设置"
  );
  check(
    "UI notice 纯 LLM 费用",
    appSrcAuto.includes("纯 LLM 费用") && appSrcAuto.includes("Auto 万能模式"),
    "notice 须明示纯 LLM 费用"
  );
  check(
    "UI 语言下拉末尾 Auto",
    appSrcAuto.includes("data-testid', 'lang-select") ||
      appSrcAuto.includes('data-testid", "lang-select'),
    "缺 lang-select"
  );
  ...
  check(
    "types 末位 auto",
    /'ja', 'auto'|"ja", "auto"/.test(typesSrc) || typesSrc.includes("'auto'"),
    "SOURCE_LANGS 末位须 auto"
  );
  ...
  // 12e) DOM 口径：lang-select 末位 auto + gloss-notice 纯 LLM + auto token .from-llm（无 .missing）
  {
    ...
    check(
      "DOM 语言末位 auto",
      opts[opts.length - 1].value === "auto",
      opts.map((o) => o.value).join(",")
    );
    check(
      "DOM auto 文案万能",
      /万能/.test(opts[opts.length - 1].textContent ?? ""),
      opts[opts.length - 1].textContent ?? ""
    );
    ...
    check(
      "DOM notice 纯 LLM 费用",
      /纯 LLM 费用/.test(notice.textContent) && /Auto/.test(notice.textContent)
    );
    ...
    renderParagraphs(holder, annAuto as never, {
      mode: "A" as never,
      showGloss: true,
      filters: { hideStopwords: false, hideKnown: false, freqHideTopN: 0 },
      showAIButton: false,
      onTokenClick: () => undefined,
      onSentenceAI: () => undefined,
    });
    check(
      "DOM auto token 有注",
      holder.querySelectorAll(".tok .gloss").length === 2,
      `got ${holder.querySelectorAll(".tok .gloss").length}`
    );
    check(
      "DOM auto 全 from-llm",
      holder.querySelectorAll(".tok.from-llm").length === 2,
      `got ${holder.querySelectorAll(".tok.from-llm").length}`
    );
